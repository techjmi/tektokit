/**
 * PrivacyPolicy - Reusable Privacy Policy page component
...
const PrivacyPolicy = ({ config, className = '', ...props }) => {
  if (!config) return null;

  const { title = 'Privacy Policy', updated, summary, sections = [] } = config;
    ...
    <main className={`py-10 ${className}`} {...props}>
      <div className="max-w-3xl mx-auto space-y-8">
        <header className="space-y-1">
          ...
          {summary && (
            <p className="text-sm md:text-base opacity-90">{summary}</p>
          )}
        </header>
